...
const SocketProvider = ({children}) => {
  ...
  global.lastDate = 0;
  global.socket = socketIO(SOCKET_URL, {
    jsonp: false,
  ...
  const onMessageReceived = (data, currentUserId) => {
    ...
    if (data.toUserId === currentUserId) {
      ...
      socket.emit('singleChatSeenMessage', {
        ...params,
        seen: true,
  ...
  const initializeSocket = async () => {
    ...
    socket.on('connect', async function (e) {
      ...
      socket.on('singleChatDelMessage', () =>
        console.log('singleMsgDelForAll'),
      );
      ...
      socket.on('singleChatReceiveMessage', data => {
        console.log('onMessageReceived'),
          onMessageReceived(data, userDetails.userId);
      });

      socket.on('singleChatDeliveryMessage', () =>
        console.log('onMessageDelivered' + userDetails.userId),
      );
    });
  };
  useEffect(() => {
