  ...
  return (
    <>
      <FlatList
        data={logs}
        renderItem={({item}) => {
          const toUser = item.userDetails.filter(
            ...
            <TouchableOpacity
              onPress={() =>
                navigation.push('Chatdetails', {
                  chatId: item._id,
                  toUser: toUser,
                  toUserId: item.userList.filter(ele => ele != user?.userId)[0],
                })
              }>
              <View style={styles.listContainer}>
                {chatmsg.filter(ele => ele.chatId === item._id)[0] && (
                  <Text style={styles.date}>
                    {new Date(
                      chatmsg.filter(
                        ele => ele.chatId === item._id,
                      )[0]?.msgTime,
                    ).toDateString()}
                  </Text>
                )}
                ...
                <View style={styles.listInfo}>
                  ...
                  <Text style={styles.listInfoSubTitle}>
                    {typing &&
                    typinguser ===
                      item.userList.filter(ele => ele != user?.userId)[0] ? (
                      'typing..'
                    ) : (
                      <>
                        <IonIcon
